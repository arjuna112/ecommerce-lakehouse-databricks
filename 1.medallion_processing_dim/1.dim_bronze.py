...
# MAGIC %md
# MAGIC # Create tables in bronze layer and ingest the data
...
# Define schema for the data file of brand dimension table
...
# assign the full path of the raw data into a variable.
raw_data_path="/Volumes/ecommerce/source_data/raw/brands/*.csv"

# Load the data from csv into the spark Data Frame using the defined schema.
df= spark.read \
    .option('header', 'true') \
    .option('delimiter', ',') \
    .schema(brand_schema) \
...
# add meta data columns in that Data Frame. 
...
# MAGIC %md
# MAGIC # from spark DF write to a Delta Table in Databricks

# COMMAND ----------

# Write raw data to Bronze layer (catalog: ecommerce, schema: bronze, table: brz_brands)
df.write.format('delta')\
    .mode("overwrite")\
    .option("mergeSchema", "true")\
    .saveAsTable(f"{catalog_name}.bronze.brz_brands") # i will call the table name as brz_brands

# COMMAND ----------

# MAGIC %md
# MAGIC # Now repeat the above SAME steps for 'remaining Dimension tables of - Category, Products, Customers, Date'
...
# write data to bronze layer (catalog: ecommerce, schema: bronze, table: brz_category)
...
# write data to bronze layer (catalog: ecommerce, schema: bronze, table: brz_products)
df.write.format("delta") \
    .mode("overwrite") \
    .option("mergeSchema", "true") \
